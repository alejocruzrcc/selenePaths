...
carpetaTodos = 'total'
if os.path.exists(carpetaTodos):
        shutil.rmtree(carpetaTodos)
        os.makedirs(carpetaTodos)
else:
        os.makedirs(carpetaTodos)
...
for est in range(len(estudiantes)):
    ...
    for f in range(0, len(filasainsertar)):
        ...
        fl = f+ fi
        row_signout = [estudiantes[est], 'Signout', 'Signout', data_collection[est]['session'].iloc[fl-1], '', '', data_collection[est]['datetime'].iloc[fl-1], '']
    ...
    for f in range(0, len(filasainsertar)):
        ...
        fl = f+ fi
        row_signin = [estudiantes[est], 'Signin', 'Signin', data_collection[est]['session'].iloc[fl], '', '', data_collection[est]['datetime'].iloc[fl], '']
    ...
    row_signin = [estudiantes[est], 'Signin', 'Signin', data_collection[est]['session'].iloc[0], '', '', data_collection[est]['datetime'].iloc[0], '']
    data_collection[est]= Insert_row(0, data_collection[est], row_signin)
    
    row_signout = [estudiantes[est], 'Signout', 'Signout', data_collection[est]['session'].iloc[numeroFilas.session], '', '', data_collection[est]['datetime'].iloc[numeroFilas.session], '']
    numeroFilas = data_collection[est].count()
...
for est in range(len(estudiantes)):
    ...
    data_collection[est]['name'] = data_collection[est]['name'].astype(str)+"_"+data_collection[est]['section'].astype(str)+"_"+data_collection[est]['subsection'].astype(str)+"_"+data_collection[est]['unit']
...
datosA['Source'] = datosA['Source'].replace(['Signin___'], 'Signin')
datosA['Source'] = datosA['Source'].replace(['Signout___'], 'Signout')
datosA['Target'] = datosA['Target'].replace(['Signin___'], 'Signin')
datosA['Target'] = datosA['Target'].replace(['Signout___'], 'Signout')
...
datosAmodulos = pd.concat([datosA.copy(), steps], axis=1, )
...
exportarCsv(datosCompleto, 'completoT', rutamodulos)
